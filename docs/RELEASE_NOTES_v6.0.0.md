...
> ⚠️ **This is not a drop-in replacement.** Method names, request/response shapes, class groupings, and error handling have all changed. Read the [Migration Guide](./MIGRATION.md) before upgrading.
...
- Every API class and method has been renamed — no mechanical find-and-replace works. See the [class-by-class mapping](./MIGRATION.md#10-class-by-class-mapping-all-36-legacy-classes).
...
- Some other legacy endpoints have no v2 equivalent (various account-verification, receipt-read, and test-receipt operations) — see [Endpoints removed in v2](./MIGRATION.md#13-endpointsmethods-removed-in-v2).
...
Full guide: [docs/MIGRATION.md](./MIGRATION.md) — covers imports, auth, base paths, the naming convention change, request/response model changes, error handling, the complete legacy→v2 class map, side-by-side examples, and a step-by-step checklist.
